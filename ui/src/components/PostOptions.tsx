    ...
    <ul className={cn('space-y-3', className)}>
      {options && options.map(o => (
        <li key={o.key}>
          ...
          <label htmlFor={o.key} className='checked-bg-bgp2 group-1 flex w-full p-2 hover:bg-bgp2  cursor-pointer rounded-xl'>
            {o.post.image ? (
                <img
                  src={o.post.image}
                ...
                <div
                  className='w-16 h-16 bg-bgs1/20 rounded-lg'
                  style={{ backgroundColor: o.post.color ? `#${uxToHex(o.post.color)}` : undefined }}
                />
              )}
            ...
            <div className='flex items-center justify-center m-5 h-6 w-6 border-2 border-solid border-fgp1 rounded'>
