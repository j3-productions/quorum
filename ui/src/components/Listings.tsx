        ...
        <li key={l.hash}>
          <article className='group-1 flex w-full'>
            {l.post.image ? (
              ...
              <div
                className='w-24 h-24 bg-bgs1/20 rounded-xl'
                style={{ backgroundColor: l.post.color ? `#${uxToHex(l.post.color)}` : undefined }}
              />
            )}
            <div className='flex-1 ml-4'>
              <a className='block w-full mb-2 hover:text-bgs1 transition-colors' href={l.post.link}>
              ...
              <div className='flex flex-col items-start sm:flex-row sm:items-center w-full font-mono text-xs text-zinc-500 opacity-0 group-1-hover:opacity-100 transition-opacity'>
                <div className='flex items-center'>
                  <span className='inline-flex items-center'>
                    ...
                    {outgoing[deSig(l.source) || ''] && (
                      <button className='group-2 relative inline-flex items-center justify-center px-2 hover:text-bgs1' aria-label='Remove pal' title="Remove Pal" onClick={() => removePal(deSig(l.source) || '')}>
                ...
                <button className='sm:ml-auto font-semibold font-sans hover:text-bgs1 bg-transparent border-0' onClick={() => remove(l.hash)}>remove</button>
              </div>
            </div>
          </article>
        </li>
